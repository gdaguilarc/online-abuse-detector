...
const rules_obj = require("../rules.json");

const twitterConfig = require("../configs/twitter");
const TwitterModel = require("../models/TweetModel");
...
async function bearerToken(auth) {
  const requestConfig = {
    url: bearerTokenURL,
    auth: {
      user: twitterConfig.consumer_key,
      pass: twitterConfig.consumer_secret,
    },
    form: {
  ...
  stream
    .on("data", async (data) => {
      try {
        ...
        await TwitterModel.create(tweet);
